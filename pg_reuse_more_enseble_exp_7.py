...
def try_model(keys, imgs, labels, model_info, round_nr, lower_bound=True):
    ...
    for key in keys:
        ...
        if lower_bound:
            mean_score = calculate_score(rounds, mean_total - std_total, old_score)
        else:
            mean_score = calculate_score(rounds, mean_total, old_score)
    ...
    mean_total /= len(keys)
    std_total /= len(keys)
    #print(f'y hat list shape: {np.asarray(y_hat_list).shape}')
    y_hat_sum = np.mean(y_hat_list, axis=tuple(range(imgs[0].ndim-2)))
    #print(f'y hat shape: {y_hat_sum.shape}')

    #print(y_hat.shape)
    score_p = pixel_score(labels[0], y_hat_sum)
    score_iou = iou_score(labels[0], y_hat_sum)



    return score_p, score_iou, mean_total, std_total
...
#
def play_one_episode(hist_keys, data_generator, model_info, value_fuction, policy_model, gamma, episode, box_size, batch_size, reuse):
    ...
    picked_keys = hist_keys#model_swich_keys[0]
    picked_values = []

    for key in picked_keys:
        picked_values.append(model_info[key])
    ...
    for i in range(batch_size):
        ...
        for picked_key in picked_keys:
            model_states_keys.append(picked_key)
        ...
        for box_index in range(box_size - reuse):
        ...
        '''
        First chouse reuse models, then choose wich one to be used one hot encoded
        ...
        #choosing the kyes that should be used in next round
        picked_keys = np.random.choice(model_states_keys, size=reuse, replace=False, p=action_prob) #possible_actions


        '''
        SER IKKE POENGET MED DENNE
        ...
        score_pixel, score_iou,  mean, std = try_model(picked_keys, img, label, model_info, episode)
    ...
    return picked_keys, total_reward, policy_loss, value_loss, pixel_scores_l, iou_scores, mean_scores, std_scores
...
file_save_path_name = f'enseble_ressults/c_batch_10_ensemble_{REUSE}_128_true_3_nodes_lower_lr.json'
