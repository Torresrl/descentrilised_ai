...
def play_one_episode(hist_keys, data_generator, model_info, value_fuction, policy_model, gamma, episode, box_size, batch_size, reuse):
    ...
    for i in range(batch_size):
        ...
        for key in picked_keys:
            key_index.append(np.where(np.asarray(model_states_keys)  == key))

        new_properbilities = []
        prop_sum = 0

        #print(f'found keys to be reused: {key_index}')

        for i in key_index:
           # print(f'Describe i: {i[0][0]}')
            prop = action_prob[i[0][0]]
            new_properbilities.append(prop)
            prop_sum += prop


        #hot encode new properbilities, (so they sum to one)
        new_properbilities[:] = [x / prop_sum for x in new_properbilities]

        #pick the key to be used
        picked_key = np.random.choice(picked_keys, p=new_properbilities) #possible_actions
